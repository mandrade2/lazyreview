...
import { runScenario } from "./flat-review"
...
test("flat review keeps selection on the next logical entry", async () => {
  const configDir = join(tmpdir(), `lazyreview-flat-review-config-${Date.now()}`)
  ...
  try {
    ...
    const navigateSnapshot = snapshots[1]!
    const markSnapshot = snapshots[2]!

    const navigatedPath = getSelectedFilePath(navigateSnapshot.spans)
    expect(navigatedPath).toBe("src/index.ts")

    const afterMarkPath = getSelectedFilePath(markSnapshot.spans)
    expect(afterMarkPath).toBe("src/legacy.ts")
  } finally {
