...
import { runScenario } from "./reviewed-list"
...
test("reviewed list keeps selection on the next logical entry after unmark", async () => {
  const configDir = join(tmpdir(), `lazyreview-reviewed-list-config-${Date.now()}`)
  ...
  try {
    ...
    const inReviewedSnapshot = snapshots[2]!
    const afterUnmarkSnapshot = snapshots[3]!

    const selectedInReviewed = getSelectedFilePath(inReviewedSnapshot.spans)
    expect(selectedInReviewed).toBe("src/components/counter.tsx")

    const selectedAfterUnmark = getSelectedFilePath(afterUnmarkSnapshot.spans)
    expect(selectedAfterUnmark).toBe("src/app.config.ts")
  } finally {
