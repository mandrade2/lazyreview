...
import { runScenario } from "./tree-review"
...
test("tree review keeps selection on the next logical entry", async () => {
  const configDir = join(tmpdir(), `lazyreview-tree-review-config-${Date.now()}`)
  ...
  try {
    ...
    expect(navigatedPath).toBe("index.ts")

    const afterMarkPath = getSelectedFilePath(markSnapshot.spans)
    expect(afterMarkPath).toBe("legacy.ts")
  } finally {
